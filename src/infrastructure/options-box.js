    ...
    render() {
        return ( 
            <div
                ...
                style={{
                    borderStyle: this.state.display? "solid" : "none"
                }}
            >
                <Button
                ...
                >
                    {this.props.title + (this.state.display? "\u25BE":"\u25B8")}
                </Button>
                {this.state.display? this.content().map((d) => <li key={d.key} style={{listStyle: "none"}}>{d}</li>) : null }
            </div>
        );
    }
}
